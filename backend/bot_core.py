...
class EitaaBot:
    ...
    def login(self, phone_number=None):
        try:
            # راه‌اندازی منابع Playwright در این درخواست (با توجه به threaded=False در app.py)
    ...
    def submit_code(self, code):
        try:
            ...
            # **اصلاح حیاتی: ارسال کد تأیید با زدن Enter**
            code_input.press('Enter') 

            # انتظار برای لود شدن صفحه اصلی (باکس جستجو)
            ...
            # مدیریت خطای تایم‌اوت Playwright
            print(f"PLAYWRIGHT TIMEOUT ERROR: {te}") 
            if self.page:
                self.page.screenshot(path='submit_code_timeout.png')
            return f"error: Playwright Timeout during login. Details: {te}"
            
        except Exception as e:
            # مدیریت خطای عمومی
            print(f"GENERAL ERROR IN SUBMIT_CODE: {e}") 
            if self.page:
                self.page.screenshot(path='submit_code_general_error.png')
            return f"error: {e}"
